...
export const describeFilter = <TRow>(node: FilterNode, fieldConfig: FilterFieldConfig<TRow>): string => {
  if (node.kind === 'condition') {
  ...
  return node.children
    .map(child =>
      child.kind === 'group' ? `(${describeFilter(child, fieldConfig)})` : describeFilter(child, fieldConfig),
    )
    .join(joiner)
}
