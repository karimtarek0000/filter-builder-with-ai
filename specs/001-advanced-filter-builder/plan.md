...
└── features/
    └── filter-builder/
        ...
        ├── filterEngine.ts               # plain functions: evaluateNode, matchCount, describeFilter (sentence), describeMatchCount (FR-032, research.md §18), hireDate component extraction — no framework imports; evaluateNode treats an invalid condition value as vacuous via validation.ts
        ...
        ├── FilterBuilder.tsx             # top-level: pure composition — useFilterUrlSync + filterEmployees, renders sentence (describeMatchCount) + FilterGroup(root) + EmployeeTable, no inline logic (FR-032, research.md §18)
        ...
        ├── FilterCondition.tsx           # renders one condition's field/operator/value controls in aligned grid columns (FR-023, single-column below `md` / fixed columns at `md`+ for FR-026) from useConditionRow's return value only — no field/operator/valueKind branching of its own (FR-029)
